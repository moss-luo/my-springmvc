	...
	draw: function() {
		...
		if (0 < f || 0 < g) {
			...
			if (0 < Math.abs(e) && 0 < Math.abs(d)) {
				...
				var x =
				AmCharts.polygon(b, [0, 0, f, f, 0], [0, e, e + g, g, 0], r, j, 0, 0, 0, p),
...
AmCharts.AmLegend = AmCharts.Class({
	...
	createMarker: function(a, b, d) {
		...
		case "triangleDown":
			...
			break;
		case "bubble":
			g = AmCharts.circle(f, e / 2, b, d, h, b, j, !0),
			g.translate(e / 2, e / 2)
		}
		return g
	},
...
AmCharts.AmBalloon = AmCharts.Class({
	...
	followCursor: function(a) {
		var b = this; (b.follow = a) ? (b.pShowBullet = b.showBullet, b.showBullet = !1) : void 0 != b.pShowBullet &&
...
AmCharts.TrendLine = AmCharts.Class({
	...
	draw: function() {
		...
		q && !u && (isNaN(s) || (g = q.getCoordinate(s)), isNaN(t) || (h = q.getCoordinate(t))); ! isNaN(e) && (!isNaN(f) && !isNaN(g) && !isNaN(g)) && (b.rotate ? (j = [g, h], f = [e, f]) : (j = [e, f], f = [g, h]), g = a.lineColor, e = AmCharts.line(d, j, f, g, a.lineAlpha, a.lineThickness, a.dashLength), f = AmCharts.line(d, j, f, g, 0.005, 5), d = d.set([e, f]), d.translate(b.marginLeftReal, b.marginTopReal),
...
AmCharts.ValueAxis = AmCharts.Class({
	...
	draw: function() {
		...
		if (this.logarithmic && (0 >= this.getMin(0, this.data.length - 1) || 0 >= this.minimum)) this.fire("logarithmicAxisFailed", {
			...
			Infinity != this.min && -Infinity != this.max) {
				...
				t = !1;
				for (e = n; e < o; e += q) v = AmCharts.roundTo(this.step * e + this.min, d),
				-1 != ("" + v).indexOf("e") && (t = !0, ("" + v).split("e"));
				this.duration && (this.maxInterval = AmCharts.getMaxInterval(this.max, this.duration));
				for (e = n; e < o; e += q) if (n = this.step * e + this.min, n = AmCharts.roundTo(n, this.maxDecCount + 1), !(this.integersOnly && Math.round(n) != n)) { ! 0 == j && (0 == n && (n = this.minReal), 2 < r && (n = Math.pow(10, e)), t = -1 != ("" + n).indexOf("e") ? !0: !1);
			...
			a = this.labelsSet;
			this.positionTitle();
			"radar" != k ? (k = this.viX, f = this.viY, b.translate(k, f), a.translate(k, f)) : j.toFront(); ! this.visible || h ? (b.hide(),
	...
	stackGraphs: function(a) {
		...
		m,
		p = this.baseValue;
		if (("line" == a || "step" == a || "smoothedLine" == a) && ("regular" == b || "100%" == b)) for (m = 0; m < j.length; m++) h = j[m],
		h.hidden || (l = h.type, h.chart == this.chart && (h.valueAxis == this && a == l && h.stackable) && (k && (h.stackGraph = k), k = h));
		...
		for (k = this.start; k <= this.end; k++) for (m = 0; m < j.length; m++) h = j[m],
		h.hidden || (l = h.type, h.chart == this.chart && (h.valueAxis == this && a == l && h.stackable) && (l = this.data[k].axes[this.id].graphs[h.id], h = l.values.value, isNaN(h) || (d = 100 * (h / g[k]), l.values.percents = d, l.values.total = g[k], "100%" == b && (isNaN(e[k]) && (e[k] = 0), isNaN(f[k]) &&
...
AmCharts.CategoryAxis = AmCharts.Class({
	...
	draw: function() {
		...
		if (AmCharts.ifArray(a)) {
			...
			for (g =
			0; g < this.data.length; g++) if (j = this.data[g]) k = this.parseDates && !this.equalSpacing ? Math.round((j.time - this.startTime) * this.stepWidth + this.cellWidth / 2) : this.getCoordinate(g - f),
			j.x[this.id] = k
		}
		j = this.guides.length;
		for (g = 0; g < j; g++) k = this.guides[g],
		n = h = d = NaN,
		k.toCategory && (n = b.getCategoryIndexByValue(k.toCategory), isNaN(n) || (d = this.getCoordinate(n - f), l = new this.axisItemRenderer(this, d, "", !0, NaN, NaN, k), this.pushAxisItem(l))),
		k.category && (n = b.getCategoryIndexByValue(k.category), isNaN(n) || (h = this.getCoordinate(n -
		f), n = (d - h) / 2, l = new this.axisItemRenderer(this, h, k.label, !0, NaN, n, k), this.pushAxisItem(l))),
		k.toDate && (this.equalSpacing ? (n = b.getClosestIndex(this.data, "time", k.toDate.getTime(), !1, 0, this.data.length - 1), isNaN(n) || (d = this.getCoordinate(n - f))) : d = (k.toDate.getTime() - this.startTime) * this.stepWidth, l = new this.axisItemRenderer(this, d, "", !0, NaN, NaN, k), this.pushAxisItem(l)),
		k.date && (this.equalSpacing ? (n = b.getClosestIndex(this.data, "time", k.date.getTime(), !1, 0, this.data.length - 1), isNaN(n) || (h = this.getCoordinate(n -
		f))) : h = (k.date.getTime() - this.startTime) * this.stepWidth, n = (d - h) / 2, l = "H" == this.orientation ? new this.axisItemRenderer(this, h, k.label, !1, 2 * n, NaN, k) : new this.axisItemRenderer(this, h, k.label, !1, NaN, n, k), this.pushAxisItem(l)),
		d = new this.guideFillRenderer(this, h, d, k),
		h = d.graphics(),
		this.pushAxisItem(d),
		k.graphics = h,
		h.index = g,
...
AmCharts.RecItem = AmCharts.Class({
	construct: function(a, b, d, e, f, g, h, j, k) {
		...
		da = S.getBBox().width;
		if ("H" == Y) {
			if (0 <= b && b <= aa + 1 && (0 < t && (0 < s && b + k <= aa + 1) && (ca = AmCharts.line(q, [b + k, b + k], [0, t], r, s, x), n.push(ca)), 0 < u)) M = AmCharts.line(q, [b, b + L, b + L], [ea, ea + F, F], y, u, x, D),
			n.push(M);
			w = 0;
			C = b;
			h && 90 == E && (C -= m); ! 1 == e ? (M = "start", w = "bottom" == K ? J ? w + t: w - t: J ? w - t: w + t, C += 3, f && (C += f / 2, M = "middle"), 0 < E && (M = "middle")) : M = "middle";
			...
			if (S && (K = 0, J && (K = da * Math.cos(R)), F + K > aa + 1 || 0 > F)) S.remove(),
			S = null
		} else {
			if (0 <= b && b <= ea +
			1 && (0 < t && (0 < s && b + k <= ea + 1) && (ca = AmCharts.line(q, [0, t], [b + k, b + k], r, s, x), n.push(ca)), 0 < u)) M = AmCharts.line(q, [0, L, aa + L], [b, b + F, b + F], y, u, x, D),
			n.push(M);
			M = "end";
			...
			J = -3;
			"right" == K && (J += F);
			if (S && (w > ea + 1 || w < J)) S.remove(),
			S = null
		}
		ca && ca.translate(fa, ha); ! 1 == a.visible && (ca && ca.remove(), S && (S.remove(), S = null));
		S && (S.attr({
			"text-anchor": M
...
AmCharts.RadItem = AmCharts.Class({
	construct: function(a, b, d, e, f, g, h) {
		...
		b = a.y - b;
		if ("polygons" == v) {
			for (var L = [], F = [], Y = a.data.length, d = 0; d < Y; d++) j = 180 - 360 / Y * d,
			L.push(b * Math.sin(j / 180 * Math.PI)),
			F.push(b * Math.cos(j /
			180 * Math.PI));
			L.push(L[0]);
			F.push(F[0]);
		...
		if (1 == f && 0 < x && !h) {
			h = a.previousCoord;
			if ("polygons" == v) {
				for (d = Y; 0 <= d; d--) j = 180 - 360 / Y * d,
				L.push(h * Math.sin(j / 180 * Math.PI)),
				F.push(h * Math.cos(j / 180 * Math.PI));
...
AmCharts.RadarFill = AmCharts.Class({
	construct: function(a, b, d, e) {
		...
		if ("polygons" == a.gridType) {
			for (var e = [], k = [], l = a.data.length, m = 0; m < l; m++) j = 180 - 360 / l * m,
...
AmCharts.AmGraph = AmCharts.Class({
	...
	createXYGraph: function() {
		...
		for (var f = this.start; f <= this.end; f++) {
			...
			k = h.y,
			h = d.getCoordinate(j),
			l = e.getCoordinate(k); ! isNaN(j) && !isNaN(k) && (a.push(h), b.push(l), (j = this.createBullet(g, h, l, f)) || (j = 0), this.labelText && (g = this.createLabel(g, h, l), this.positionLabel(h, l, g, this.labelPosition, j)))
		}
	...
	createSerialGraph: function() {
		...
		for (N = O; N <= E; N++) {
			...
			if (na) {
				...
				case "line":
					...
					else if (P < F && (w.isNegative = !0), o.push(A), r.push(z), V = A, W = z, ra = A, sa = z, Ra) ka.push(X),
					la.push(T);
					break;
				case "smoothedLine":
					...
					if (!isNaN(P)) {
						...
						if (ja) {
							...
							if (!U) {
								...
								if ("regular" == v || "100%" == v) B = "middle",
								x = m ? 0 < na.value ? (this.end + 1 - N) * u + b: (this.end + 1 - N) * u - b: 0 < na.value ? u * N + b: u * N - b;
				...
				case "candlestick":
					if (!isNaN(ia) && !isNaN(Ta) && !isNaN(Sa) && !isNaN(P)) {
						...
						if (ja && (oa = ja.set, ba.push(oa), oa.translate(I, H), w.url && oa.setAttr("cursor", "pointer"), Ma && (ba.push(Ma), ba.push(Va)), V = A, W = z, ra = A, sa = z, !U)) w.x = m ? H + G / 2: I + G / 2,
						this.animateColumns(ja, N, A, X, z, T),
						this.addListeners(oa, w)
					}
					break;
					...
					if (this.end - this.start <= mb || 0 == mb) {
						...
						if (this.labelText) {
							...
							if (ga) if (V += ua, W += Ba, ga.translate(V, W), m) {
								if (0 > W || W > k) ga.remove(),
								ga = null
							} else if (0 > V || V > j) ga.remove(),
							ga = null
						}
					}
				}
		...
		if ("line" == g || "step" == g || "smoothedLine" == g)"smoothedLine" == g ? this.drawSmoothedGraph(o, r, ka, la) : this.drawLineGraph(o, r, ka, la),
		U || this.launchAnimation()
	},
	...
	drawLineGraph: function(a, b, d, e) {
		if (1 < a.length) {
			...
			r = this.baseCoord,
			s = AmCharts.line(g, a, b, this.lineColor, k, l, m, !1,
			!0);
			h.push(s);
			void 0 != q && (k = AmCharts.line(g, a, b, q, k, l, m, !1, !0), j.push(k));
			if (0 < f && (k = a.join(";").split(";"), l = b.join(";").split(";"), "serial" == this.chartType && (0 < d.length ? (d.reverse(), e.reverse(), k = a.concat(d), l = b.concat(e)) : this.rotate ? (l.push(l[l.length - 1]), k.push(r), l.push(l[0]), k.push(r), l.push(l[0]), k.push(k[0])) : (k.push(k[k.length - 1]), l.push(r), k.push(k[0]), l.push(r), k.push(a[0]), l.push(l[0]))), a = AmCharts.polygon(g, k, l, p, f), h.push(a), n || void 0 != q)) o || (o = f),
			n || (n = q),
			g = AmCharts.polygon(g, k, l,
			n, o),
			j.push(g);
			this.applyMask(j, h)
		}
	...
	drawSmoothedGraph: function(a, b) {
		if (1 < a.length) {
			...
			if (0 < k && (h = "", this.rotate ? (h += " L" + n + "," + b[b.length - 1], h += " L" + n + "," + b[0]) : (h += " L" + a[a.length - 1] + "," + n, h += " L" + a[0] + "," + n), h += " L" + a[0] + "," + b[0], l = new AmCharts.Bezier(e, a, b, NaN, 0, 0, l, k, d, h), f.push(l.path), p || void 0 != m)) q || (q = k),
			p || (p = m),
			e = new AmCharts.Bezier(e, a, b, NaN, 0, 0, p, q, d, h),
			g.push(e.path);
			this.applyMask(g, f)
		}
	},
	...
	createBullet: function(a, b, d) {
		...
		if (this.customBullet || a.customBullet) if (f = this.customBullet, a.customBullet && (f = a.customBullet), f) j && j.remove(),
		"function" == typeof f ? (j = new f, j.chart = this.chart, a.bulletConfig && (j.availableSpace = d, j.graph = this, a.bulletConfig.minCoord = this.minCoord - d, j.bulletConfig = a.bulletConfig), j.write(e), j = j.set) :
		(this.chart.path && (f = this.chart.path + f), j = e.image(f, 0, 0, g, g).attr({
			preserveAspectRatio: !0
		}), this.centerCustomBullets && (b -= g / 2, d -= g / 2));
		if (j) {
			a.url && j.setAttr("cursor", "pointer");
			this.allBullets.push(j);
			if ("serial" == this.chartType && (0 > b || b > this.width || d < -g / 2 || d > this.height)) j.remove(),
			j = null;
...
AmCharts.ChartCursor = AmCharts.Class({
	...
	updateSelectionSize: function(a, b) {
		...
		h = 0;
		if (!isNaN(b) && (e > b && (g = b, j = e - b), e < b && (g = e, j = b - e), e == b)) g = b,
		j = 0;
		0 < h && 0 <
		...
		if (this.enabled) {
			...
			if (d.mouseIsOver || v || B || this.forceShow) if (this.forceShow = !1, B) {
				...
				if (O && this.valueBalloonsEnabled && u && d.balloon.enabled) {
					...
					U, u = 0; u < O.length; u++) s = O[u],
					s.showBalloon && (!s.hidden && s.balloonText) && (t = q.axes[s.valueAxis.id].graphs[s.id], x = t.y, isNaN(x) || (o ? Math.abs(e - x) < h && (h = Math.abs(e - x), U = s) : Math.abs(f - x) < h && (h = Math.abs(f - x), U = s)));
				...
				m.index = a, m.target = this, m.chart = this.chart, m.zooming = v, m.position = o ? f: e, m.target = this, d.fire("changed", m),
	...
	updateAnimations: function() {
		for (var a = this.animations.length - 1; 0 <= a; a--) {
			...
			if (e <= d) {
				...
				j = Number(j[1]),
				k = 0 ==
				k - g ? k: Math.round(this.D[b.effect](0, e, g, k - g, d)),
				...
				this.setAttr(f, g, b)
			} else "translate" == g ? (j = b.to.split(","), k = Number(j[0]), j = Number(j[1]), f.translate(k, j)) : (k = Number(b.to), this.setAttr(f, g, k)),
			this.animations.splice(a, 1)
		}
	},
	getBBox: function(a) {
