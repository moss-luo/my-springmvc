...
function setPoptPageInfo() {
	...
	poptfouhref = "SummarizePageInfo.jsp?productid=" + productid + "&platformid=" + platformid + "&platformtype=" + vplatformtype + "&platforms=" + vplatforms + "&liid=adtop4";
...
var productid = $("#productTxt").val();
	...
	poptthrhref = "SummarizeStartUpInfo.jsp?productid=" + productid + "&platformid=" + platformid + "&platformtype=" + vplatformtype + "&platforms=" + vplatforms + "&liid=adtop3";
	poptfouhref = "SummarizePageInfo.jsp?productid=" + productid + "&platformid=" + platformid + "&platformtype=" + vplatformtype + "&platforms=" + vplatforms + "&liid=adtop4";
	poptfivehref = "SummarizeCustomEvent.jsp?productid=" + productid + "&platformid=" + platformid + "&platformtype=" + vplatformtype + "&platforms=" + vplatforms + "&liid=adtop5";
	...
	popteighthref = "SummarizeUseTimeInfo.jsp?productid=" + productid + "&platformid=" + platformid + "&platformtype=" + vplatformtype + "&platforms=" + vplatforms + "&liid=adtop8";
	poptnighthref = "SummarizeVersionInfo.jsp?productid=" + productid + "&platformid=" + platformid + "&platformtype=" + vplatformtype + "&platforms=" + vplatforms + "&liid=adtop9";
				...
				"fnRender" : function(obj) {
					...
					if(type == '今日' || type == '昨日') {
						...
						if(type == '预计今日') {
							...
							if(parseInt(ordertoday) > parseInt(yesterday)) {
				...
				"fnRender" : function(obj) {
					...
					if(type == '今日' || type == '昨日') {
						...
						if(type == '预计今日') {
							...
							if(parseInt(ordertoday) > parseInt(activeyesterday)) {
								info = "<div class='relative'>" + info + "<font>↑</font></div>";
							} else {
								info = "<div class='relative'>" + info + "<font>↓</font></div>";
							}
...
function setBaseDataTable(data) {
	if(isEmpty(data)) {
		...
		if(isEmpty(tmpGridInfo)) {
			if(tmpGridInfo.data900510){
				$('#900510').html(fmoney(tmpGridInfo.data900510));
				tmpAllUser=parseInt(tmpGridInfo.data900510);
			}else{
				$('#900510').html("0");
				tmpAllUser=0;
			}
			...
			if(sevenTimeFlag==1){
				if(tmpGridInfo.data000713){
					if(tmpAllUser!=0){
						tmpAllUser=Math.round((tmpGridInfo.data000713/tmpAllUser)*100);
					}
					$('#000713').html(fmoney(tmpGridInfo.data000713)+' <font style="font-size:15px;">('+tmpAllUser+'%)</font>');
				}
			}else{
				$('#000713').html('--');
			}
	...
	if(listKey != 1) {
		...
		if(thrDaybarModel != null && thrDaybarModel != "") {
			...
			if(contrastnewuser != null) {
				var tmpkeepuser = new Array();
				var i = 0;
				for(var c in contrastnewuser) {
					i = contrastactiveuser[c] - contrastnewuser[c];
					if(i < 0) {
						i = 0;
					}
					tmpkeepuser.push(i);
